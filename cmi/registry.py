...
from utils import bad_datasets
...
NORMALIZE_DICT = {
    ...
    'gtsrb':  dict( mean=(0.3403, 0.3121, 0.3214), std=(0.2724, 0.2608, 0.2669) ),
...
MODEL_DICT = {
    ...
    'WRN-16-2': classifiers.wresnet.wrn_16_2,
...
def get_dataset(name: str, data_root: str='data', return_transform=False, split=['A', 'B', 'C', 'D'],
                trigger_pattern='badnet_grid', poi_target=0):
    name = name.lower()
    ...
    if name=='mnist':
        ...
        data_root = DATA_PATHS[name.upper()]
        ...
        data_root = DATA_PATHS[name.upper()]
        train_dst = GTSRB(data_root, train=True, download=True, transform=train_transform)
        val_dst = GTSRB(data_root, train=False, download=True, transform=val_transform)
        poi_val_dst = bad_datasets.CIFAR_BadNet(data_root, dataset_name='gtsrb', split='test', triggered_ratio=1.,
                            trigger_pattern=trigger_pattern, target=poi_target, transform=val_transform)
    elif name=='cifar10':
        num_classes = 10
        train_transform = T.Compose([
        ...
        data_root = DATA_PATHS[name.upper()]  # os.path.join( data_root, 'torchdata' ) 
        ...
        poi_val_dst = bad_datasets.CIFAR_BadNet(data_root, dataset_name='cifar10', split='test', triggered_ratio=1.,
                            trigger_pattern=trigger_pattern, target=poi_target, transform=val_transform)
    elif name=='c10+p365':
        num_classes = 10
        train_transform = T.Compose([
        ...
        data_root = DATA_PATHS['SVHN']  # os.path.join( data_root, 'torchdata' ) 
    ...
    if return_transform:
        return num_classes, train_dst, val_dst, poi_val_dst, train_transform, val_transform
    return num_classes, train_dst, val_dst, poi_val_dst
